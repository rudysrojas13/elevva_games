...
async function matchCatalog() {
  const sellerId = '432476';
  const url = `https://api.digiseller.com/api/shop/products?seller_id=${sellerId}&rows=1000&currency=USD&lang=en-US`;
  
  console.log(`Fetching all products for seller ${sellerId}...`);
  const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
  const data = await res.json();
  
  if (data.retval !== 0) {
    console.error('Error fetching from Digiseller API:', data.retdesc);
    return;
  }
  
  const sellerProducts = data.product || [];
  console.log(`Total products in seller catalog: ${sellerProducts.length}`);
  ...
  for (const game of userGamesList) {
    ...
    for (const sp of sellerProducts) {
      ...
      // If the normalized game name is found inside the seller product name
      if (normSp.includes(normGame) || normGame.includes(normSp)) {
        ...
        found = true;
        break; // Stop looking for this game once matched once
      }
    }
  ...
  console.log('\n=== MATCH STATISTICS ===');
  console.log(`Games in User List: ${userGamesList.length}`);
  console.log(`Games Found in Seller Catalog: ${matches.length}`);
  console.log(`Games NOT Found: ${missing.length}`);

  // Write detailed report to JSON
  fs.writeFileSync('C:\\Users\\pc\\Downloads\\catalog_match_report.json', JSON.stringify({
    stats: {
      totalUserGames: userGamesList.length,
  ...
  console.log('\nDetailed match report saved to: C:\\Users\\pc\\Downloads\\catalog_match_report.json');
}

matchCatalog().catch(console.error);
